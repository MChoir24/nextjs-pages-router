...
export default async function handler(
  ...
  try {// Add logic to determine which pages to revalidate
    switch (req.query.data) { // Add more cases as needed
      case "products":
        await res.revalidate("/products/static");
        return res.json({ revalidated: true, message: "Revalidation successful" });
      default:
        return res.status(400).json({ revalidated: false, message: "Invalid data parameter" });
    }
  } catch (error) {
